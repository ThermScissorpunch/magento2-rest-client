...
module.exports = function (restClient) {
    ...
    module.setPaymentMethod = function (customerToken, cartId, body, adminRequest = false) {
        if (adminRequest) {
            return restClient.post('/carts/' + cartId + '/selected-payment-method', body);
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.post('/carts/mine/selected-payment-method', body, customerToken);
            } else
            {
                return restClient.post('/guest-carts/' + cartId + '/selected-payment-method', body);
            }
        }
    }
    ...
    module.adyenPaymentMethods = function (customerToken, cartId, address) {
        if (customerToken && isNumeric(cartId)) {
            return restClient.post('/carts/mine/retrieve-adyen-payment-methods', { address: address, cart_id: cartId })
        } else
        {
            return restClient.post('/guest-carts/' + cartId + '/retrieve-adyen-payment-methods', { address: address })
        }
    }
