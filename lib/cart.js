...
module.exports = function (restClient) {
    ...
    module.setPaymentMethod = function (customerToken, cartId, body, adminRequest = false) {
        if (adminRequest) {
            return restClient.put('/carts/' + cartId + '/selected-payment-method', body);
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.put('/carts/mine/selected-payment-method', body, customerToken);
            } else
            {
                return restClient.put('/guest-carts/' + cartId + '/selected-payment-method', body);
            }
        }
    }

    module.upsAccessPoint = function (customerToken, cartId, body, adminRequest = false) {
        if (adminRequest) {
            return restClient.post('/carts/' + cartId + '/ups-access-point', body);
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.post('/carts/mine/ups-access-point', body, customerToken);
            } else
            {
                return restClient.post('/guest-carts/' + cartId + '/ups-access-point', body);
            }
        }
    }

    module.pickupPoint = function (customerToken, cartId, body, adminRequest = false) {
        if (adminRequest) {
            return restClient.post('/carts/' + cartId + '/pickup-point', body);
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.post('/carts/mine/pickup-point', body, customerToken);
            } else
            {
                return restClient.post('/guest-carts/' + cartId + '/pickup-point', body);
            }
        }
    }

    module.order = function (customerToken, cartId, body, adminRequest = false, ip) {
        let customHeaders = {};
        if (ip) {
            customHeaders = { 'x-forwarded-for': ip };
        }
        if (adminRequest) {
            return restClient.put('/carts/' + cartId + '/order', body, '', customHeaders);
        } else {
            if (customerToken && isNumeric(cartId)) {
                return restClient.put('/carts/mine/order', body, customerToken, customHeaders);
            } else
            {
                return restClient.put('/guest-carts/' + cartId + '/order', body, '', customHeaders);
            }
        }
    }
    ...
    module.adyenPaymentMethods = function (customerToken, cartId, address) {
        if (customerToken && isNumeric(cartId)) {
            return restClient.post('/carts/mine/retrieve-adyen-payment-methods', { country: address.countryId.toLowerCase(), cart_id: cartId })
        } else
        {
            return restClient.post('/guest-carts/' + cartId + '/retrieve-adyen-payment-methods', { country: address.countryId.toLowerCase() })
        }
    }

    module.adyenPaymentStatus = function (orderId) {
        return restClient.post('/adyen/orders/carts/mine/payment-status', { orderId: orderId })
    }

    module.adyenPaymentDetails = function (details, orderId, customerToken, cartId) {
        const payload = JSON.stringify({ details, orderId });
        if (customerToken && isNumeric(cartId)) {
            return restClient.post('/adyen/carts/mine/payments-details', {
                payload,
                orderId,
                customerToken
            })
        } else {
            return restClient.post('/adyen/guest-carts/' + cartId + '/payments-details', {
                payload,
                orderId
            })
        }
    ...
    module.restoreFromOrder = function (customerToken, cartId, incrementId) {
        if (!incrementId) {
            throw new Error('Increment id cannot be empty');
        }
        if (customerToken) {
            return restClient.post(`/carts/mine/restore_from_order/${incrementId}`, {
                customer_id: customerToken
            });
        } else
        {
            return restClient.post(`/guest-carts/${cartId}/restore_from_order/${incrementId}`);
        }
    }
